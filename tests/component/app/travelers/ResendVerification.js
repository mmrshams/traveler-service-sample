...
describe(`Calling POST /travelers/verification/resend`, () => {
  let request
  beforeEach(async () => {
  ...
  describe('When required params are provided', () => {
    it('Must return status code 204', async () => {
      ...
      const { status } = await request.post('/travelers/verification/resend').send(fields)
  ...
  describe('When email field is missing', () => {
    it('Must return status code 400', async () => {
      ...
      const travelerEmail = new TravelerEmail({ travelerId: traveler.id, email: traveler.data.email })
        ...
        .withRecovery()
      await firestore.create([traveler, travelerEmail])
      const { status } = await request.post('/travelers/verification/resend').send({})
